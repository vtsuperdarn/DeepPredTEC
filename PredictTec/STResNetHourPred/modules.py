...
def Fusion(closeness_output, period_output, trend_output, scope, shape):
    ...
    with tf.variable_scope(scope):
        ...
        print "closenessss" , (closeness_output).shape
        ...
        output = tf.reshape(closeness_output, shape1)
        output = tf.matmul(output, Wc)
        closeness_output = tf.reshape(output, [closeness_output.shape[0], closeness_output.shape[1], closeness_output.shape[2], closeness_output.shape[3]])
        
        output = tf.reshape(period_output, shape2)
        output = tf.matmul(output, Wp)
        period_output = tf.reshape(output, [period_output.shape[0], period_output.shape[1], period_output.shape[2], period_output.shape[3]])
        
        output = tf.reshape(trend_output, shape3)
        output = tf.matmul(output, Wt)
        trend_output = tf.reshape(output, [trend_output.shape[0], trend_output.shape[1], trend_output.shape[2], trend_output.shape[3]])
       
        outputs = tf.add(tf.add(closeness_output, period_output), trend_output)
        
        #TODO have to check if I need to do this: converting the dimension from (B, O, H, W) -> (B, H, W, O)
