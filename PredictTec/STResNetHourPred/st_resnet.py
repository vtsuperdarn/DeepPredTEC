...
class Graph(object):
    def __init__(self):
        ...
        with self.graph.as_default():
            ...
            #Number of output filters is equal to 12 because we want to predict next one hour tec maps
            self.closeness_output = my.ResOutput(inputs=self.closeness_output, filters=O, kernel_size=(7, 7), scope="resnet_output", reuse=None)
            ...
            self.period_output = my.ResOutput(inputs=self.period_output, filters=O, kernel_size=(7, 7), scope="resnet_output", reuse=True)
            ...
            self.trend_output = my.ResOutput(inputs=self.trend_output, filters=O, kernel_size=(7, 7), scope="resnet_output", reuse=True)
            ...
            self.exo_close = tf.layers.conv2d(inputs=self.close_concat, filters=O, kernel_size=(7, 7), strides=(1,1), padding="SAME", name="exo_close") 
            self.exo_period = tf.layers.conv2d(inputs=self.period_concat, filters=O, kernel_size=(7, 7), strides=(1,1), padding="SAME", name="exo_period") 
            self.exo_trend = tf.layers.conv2d(inputs=self.trend_concat, filters=O, kernel_size=(7, 7), strides=(1,1), padding="SAME", name="exo_trend") 
            ...
            #TODO recheck the seq2seq loss and training for this again
            '''
            self.total_loss = []
            for i in range(self.x_res.shape[0]):
                loss = tf.reduce_sum(tf.pow(self.x_res[i] - self.output_tec[i], 2)) / tf.cast((self.x_res.shape[0]), tf.float32)
