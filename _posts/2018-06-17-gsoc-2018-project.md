...
## Phase 3
In the phase 3 of the project, the data pipeline was completely written in object-oriented manner for loading data easily. Complete code testing was performed by checking intermediate output values. Dask implementation was incorporated into the data pipeline for enabling lazy loading during batch creation. IMF data is normalized and lots of variants of the ST-ResNet model are trained to tune the hyperparameters. Prediction scripts and learning curves are plotted. Final code cleaning and documentation is completed.   
