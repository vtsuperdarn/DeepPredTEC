...
The training loss and validation loss values are used for plotting the loss curve for better the model performance. We plot train and validation loss over the number of epochs to understand when the model overfits on the given data. 

### Plotting the learning curve
Learning curve helps us to understand the model's performance given the model complexity and the size of the input data during training. We train a variant of the ST-ResNet model on varying input TEC data for training and get the learning curve which is average training loss vs the input data size for training.  
